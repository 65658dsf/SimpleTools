...
<template>
  ...
  <section class="drop-zone" :class="{ dragging }" @dragenter.prevent="onDragEnter" @dragover.prevent="dragging = true" @dragleave.prevent="onDragLeave" @drop.prevent="onDrop" @click="browse">
    <div class="upload-icon"><Upload :size="22" /></div><h2>{{ t('dropTitle') }}</h2><p>{{ t('dropHint') }}</p><div class="drop-actions"><button class="secondary-button" @click.stop="browse"><Plus :size="16" /> {{ t('browse') }}</button><button class="secondary-button" @click.stop="browseFolder"><FolderTree :size="16" /> {{ t('folder') }}</button></div><input ref="input" class="hidden-input" type="file" multiple :accept="store.activeTool === 'pdf' ? '.pdf,application/pdf' : '.png,.jpg,.jpeg,.webp,.avif,.ico,.svg,image/*'" @change="onInputChange" />
  </section>
  <div class="workspace-grid">
    <section class="queue-panel panel"><div class="panel-header"><div><h2>{{ t('queue') }}</h2><span class="muted">{{ store.files.length }} {{ t('files') }}</span></div><div class="queue-header-actions"><button v-if="store.files.some(item => item.status === 'error')" class="text-button" :disabled="store.running" @click="store.retryFailed"><RotateCcw :size="14" /> {{ t('retryFailed') }}</button><button v-if="store.files.some(item => item.status === 'done')" class="text-button" :disabled="store.running" @click="store.clearCompleted"><Trash2 :size="14" /> {{ t('clearCompleted') }}</button><button v-if="store.files.length" class="text-button" :disabled="store.running" @click="store.clearFiles"><Trash2 :size="15" /> {{ t('clear') }}</button></div></div>
      <div v-if="store.files.length" class="queue-filters" role="group" :aria-label="t('queueFilter')"><button v-for="filter in queueFilters" :key="filter.value" class="queue-filter" :class="{ selected: store.queueFilter === filter.value }" :aria-pressed="store.queueFilter === filter.value" @click="store.queueFilter = filter.value"><span>{{ t(filter.label) }}</span><span class="queue-filter-count">{{ filterCount(filter.value) }}</span></button></div>
